package mx.random.forms
{
	...
	public class Validator{
		...
		private static var _hourFormatString:String 		= "##:##";
		...
		public static const HOUR_VAL:String			= "hour_val";
		...
		public static const ARROW_KNOT:String 		= "arrow_knot";
		...
		private function onFocusOut(e:Event):void{
			validateAll();//Valid.validateOne(e.currentTarget.name);
		}
		...
		public function addStringVal(o:UIComponent,min:Number = 0,max:Number = 255,errorName:String = ""):void{
			...
			if(_validateChange) o.addEventListener(Event.CHANGE,onFocusOut);
			addListeners(o);
		}
		...
		public function addHourVal(o:UIComponent,format:Boolean,required:Boolean = true,errorName:String = ""):void{
			o["maxChars"] = _hourFormatString.length;
			_fields.addItem({obj:o,type:HOUR_VAL,params:{format:format,required:required,errorName:errorName}});
			if(_validateChange) o.addEventListener(Event.CHANGE,onFocusOut);
			addListeners(o);
		}
		...
		public function addNumberVal(o:UIComponent,format:Boolean = false,required:Boolean = false,precision :Number = 2,zero:Boolean=false,errorName:String = ""):void{
			_fields.addItem({obj:o,type:NUMBER_VAL,params:{format:format,required:required,precision:precision,zero:zero,errorName:errorName}});			
			if(_validateChange) o.addEventListener(FocusEvent.FOCUS_OUT,onFocusOut);
			addListeners(o);
		}
			...
			for(var n:Number = 0;n<_fields.length;n++){
				switch(_fields[n].type){
					...
					case PHONE_VAL:
						...
						break;
					case HOUR_VAL:
						if(!hourVal(_fields[n].obj,_fields[n].params.format,_fields[n].params.required)) {
							res = false;
							if(_fields[n].params.errorName!="") errors += "  - "+_fields[n].params.errorName+"\n";
						}
						break;
					...
					case MAIL_VAL:
						...
						break;
					case NUMBER_VAL:
						if(!numberVal(_fields[n].obj,_fields[n].params.format,_fields[n].params.required,_fields[n].params.precision,_fields[n].params.zero)) {
							res = false;
							if(_fields[n].params.errorName!="") errors += "  - "+_fields[n].params.errorName+"\n";
						}
						break;
		...
		public function removeRequiredKnots(group:Number):void{
			if(group==-1){
				...
				for(var m:Number = 0;m<fields.length;m++){						
					if(fields[m].obj.getChildByName("RK")) fields[m].obj.removeChild(fields[m].obj.getChildByName("RK"));				
				}
			}
		}
		...
		public   function changeLanguage(language:String):void{
			switch(language){
				...
				case Languages.SPANISH:
					...
					validCC.requiredFieldError	= "Campo Requerido";
					
					validPN.wrongLengthError	= "Número inválido";
					validPN.requiredFieldError	= "Campo Requerido";
					validPN.invalidCharError	= "Caracteres Inválidos";
					
					validST.requiredFieldError  = "Campo Requerido";
					validST.tooLongError  		= "Texto muy largo";
					validST.tooShortError  		= "Texto muy corto";
					
					validEM.requiredFieldError  		= "Campo Requerido";
					validEM.invalidCharError			= "Caracteres Inválidos";
					validEM.invalidDomainError			= "Mail Inválido";
					validEM.invalidIPDomainError		= "Mail Inválido";
					validEM.invalidPeriodsInDomainError	= "Mail Inválido";
					validEM.missingAtSignError			= "Falta '@' en el mail";
					validEM.missingPeriodInDomainError	= "Falta '.' en el mail";
					validEM.missingUsernameError		= "Falta nombre de usuario";
					validEM.tooManyAtSignsError			= "Existe mas de una '@' en el mail";
					
					validNV.requiredFieldError			= "Campo Requerido";
					validNV.invalidCharError			= "Caracteres Inválidos";
					validNV.exceedsMaxError				= "Numero muy alto";
					validNV.invalidFormatCharsError		= "Caracteres Inválidos";
					validNV.lowerThanMinError			= "Número muy bajo";
					validNV.negativeError				= "El número no puede ser negativo";
					validNV.precisionError				= "El número contiene más decimales de las permitidas";
					validNV.separationError				= "El número no puede contener espacios";
					
					validED.requiredFieldError			= "Campo Requerido";
					validED.invalidCharError			= "Caracteres Inválidos";
					validED.wrongYearError				= "Año Inválido";
					validED.wrongDayError				= "Día Inválido";
					validED.wrongLengthError			= "La longitud de la fecha es inválida";
					validED.wrongMonthError				= "Mes Inválido";
					
					validCB.requiredFieldError			= "Campo Requerido";
		...
		public   function numberVal(o:Object,format:Boolean = false,required:Boolean = false,precision:Number =2,zero:Boolean = false):Boolean{
			validNV.source = o;
			validNV.property = "text";
			validNV.required = required;
			
			var tempNumber:String = o["text"];
			var myPattern:RegExp = /[^0-9 ^. ^-]/g;
			o["text"] = tempNumber.replace(myPattern,"");
			var isZero:Boolean = false;
			if(validNV.validate().type == "valid"&&format){
				if(Number(o["text"])==0) isZero = true;
		 		o["text"] = numberFormat(o["text"],precision);
		 		if(o["text"]=="") {		 		
			 		o["text"] = tempNumber.replace(myPattern,"");			 					 		
		 		}
		 		o.setSelection(o["text"].length,o["text"].length);
		...
		public   function numberFormat(number:String,precision:Number = 2):String{
		...
		public   function hourVal(o:Object,format:Boolean = false,required:Boolean = false):Boolean{
			validPN.source = o;
			validPN.property = "text";
			validPN.required = required;
			
			var tempHour:String = o["text"];
			var myPattern:RegExp = /[^0-9]/g;
			
			o["text"] = tempHour.replace(myPattern,"");
		 	if(validPN.validate().type == "valid"&&format){
		 		o["text"] = hourFormat(o["text"]);
		 		if(o["text"]=="") {		 		
			 		o["text"] = tempHour.replace(myPattern,"");			 					 		
		 		}
		 		o.setSelection(o["text"].length,o["text"].length);
		...
		public   function hourFormat(hour:String):String{
			var form:PhoneFormatter = new PhoneFormatter();
			form.formatString 		= _hourFormatString;
			return form.format(hour);
		}
